...
namespace OpenTap.Plugins.PNAX
{
    ...
    public class StoreData : StoreDataBase
    {
        ...
        [Display(
            ...
            Description: "Generate one Result file per Channel.",
            "Measurements",
            Order: 20
        )]
        ...
        [Display(
            "Enable Limits?",
            "Choose if you want limit checking and Pass/Fail",
            "Measurements",
            Order: 30
        )]
        ...
        [Display(
            "Limits File",
            "Insert .csv file containing limits to check against (*.csv)",
            "Measurements",
            Order: 40
        )]
        ...
        public StoreData()
        {
            ...
            Rules.Add(
                () => ((RoundXNumberDigits >= 0) && (RoundXNumberDigits <= 16)),
                ...
                nameof(RoundXNumberDigits)
            );
            Rules.Add(
                () => ((RoundYNumberDigits >= 0) && (RoundYNumberDigits <= 16)),
                "The value of the digits argument can range from 0 to 15. The maximum number of integral and fractional digits supported by the Double type is 15.",
                nameof(RoundYNumberDigits)
            );
        }
        ...
        public override void Run()
        {
            ...
            try
            {
                if (GroupByChannel)
                {
                    ...
                    {
                        List<List<string>> results = PNAX.StoreTraceData(
                            new List<int>() { channel }
                        );
                        ...
                        xResult = results
                            .Where(
                                (item, index) =>
                                    ((index == 0) || ((index >= 7) && (index % 7 == 0)))
                            )
                            .ToList();
                        yResult = results
                            .Where(
                                (item, index) =>
                                    ((index == 1) || ((index >= 8) && (index % 7 == 1)))
                            )
                            .ToList();
                        ...
                        x1 = results
                            .Where(
                                (item, index) =>
                                    ((index == 3) || ((index >= 10) && (index % 7 == 3)))
                            )
                            .ToList();
                        x2 = results
                            .Where(
                                (item, index) =>
                                    ((index == 4) || ((index >= 11) && (index % 7 == 4)))
                            )
                            .ToList();
                        x3 = results
                            .Where(
                                (item, index) =>
                                    ((index == 5) || ((index >= 12) && (index % 7 == 5)))
                            )
                            .ToList();
                        x4 = results
                            .Where(
                                (item, index) =>
                                    ((index == 6) || ((index >= 13) && (index % 7 == 6)))
                            )
                            .ToList();
                        ...
                        for (var i = 0; i < traces.Count; i++)
                        {
                            if (i == 0)
                            {
                                ...
                                if (RoundXValues)
                                {
                                    resultColumn = new ResultColumn(
                                        ...
                                        xResult[i]
                                            .Select(double.Parse)
                                            .Select(x => Math.Round(x, RoundXNumberDigits))
                                            .ToArray()
                                    );
                                }
                                else
                                {
                                    resultColumn = new ResultColumn(
                                        "Frequency (Hz)",
                                        xResult[i].Select(double.Parse).ToArray()
                                    );
                                }
                            ...
                            if (xResult[i].Count == yResult[i].Count)
                            {
                                ...
                                if (RoundYValues)
                                {
                                    resultColumn = new ResultColumn(
                                        ...
                                        yResult[i]
                                            .Select(double.Parse)
                                            .Select(x => Math.Round(x, RoundYNumberDigits))
                                            .ToArray()
                                    );
                                }
                                else
                                {
                                    resultColumn = new ResultColumn(
                                        $"{TraceName}",
                                        yResult[i].Select(double.Parse).ToArray()
                                    );
                                }
                                resultColumns.Add(resultColumn);
                            }
                            else
                            {
                                ...
                                ResultColumn resultColumni = new ResultColumn(
                                    $"{TraceName}_i",
                                    point1
                                );
                                resultColumns.Add(resultColumni);
                                ResultColumn resultColumnj = new ResultColumn(
                                    $"{TraceName}_j",
                                    point2
                                );
                        ...
                        if ((MetaData != null) && (MetaData.Count > 0))
                        {
                            ...
                            foreach (var i in MetaData)
                            {
                                ...
                                ResultColumn resultColumnMeta = new ResultColumn(
                                    i.Item1,
                                    objMetaData
                                );
        ...
        {
            if (string.IsNullOrEmpty(LimitsFile))
                return false;
