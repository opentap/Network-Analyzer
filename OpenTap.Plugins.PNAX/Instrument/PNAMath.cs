...
namespace OpenTap.Plugins.PNAX
{
    ...
    public partial class PNAX : ScpiInstrument
    {
        ...
        {
            int state = IsModelA
                ? ScpiQuery<int>($"CALCulate{Channel}:LIMit:STATe?")
                : ScpiQuery<int>($"CALCulate{Channel}:MEASure{mnum}:LIMit:STATe?");
            return state == 1;
        }
        ...
        public void SetLimitData(int Channel, int mnum, List<LimitSegmentDefinition> limitSegments)
        {
            ...
            foreach (LimitSegmentDefinition limit in limitSegments)
            {
                ...
                ScpiCommand(
                    $"CALCulate{Channel}:MEASure{mnum}:LIMit:SEGMent{segm}:STIMulus:STARt {limit.BeginStim}"
                );
                ScpiCommand(
                    $"CALCulate{Channel}:MEASure{mnum}:LIMit:SEGMent{segm}:STIMulus:STOP {limit.EndStim}"
                );
                ScpiCommand(
                    $"CALCulate{Channel}:MEASure{mnum}:LIMit:SEGMent{segm}:AMPLitude:STARt {limit.BeginResp}"
                );
                ScpiCommand(
                    $"CALCulate{Channel}:MEASure{mnum}:LIMit:SEGMent{segm}:AMPLitude:STOP {limit.EndResp}"
                );
                segm++;
            }
        }
        ...
        public void MathShowResistance(int Channel, int mnum, bool state)
        {
            string stateValue = state ? "ON" : "OFF";
            ScpiCommand(
                $"CALCulate{Channel}:MEASure{mnum}:FUNCtion:STATistics:RESistance:STATe {stateValue}"
            );
        }

        public void MathStatisticsRange(
            int Channel,
            int mnum,
            MathStatisticsRangeEnum mathStatisticsRange
        )
        {
            string StatisticRange = Scpi.Format("{0}", mathStatisticsRange);
            ScpiCommand(
                $"CALCulate{Channel}:MEASure{mnum}:FUNCtion:DOMain:USER:RANGe {StatisticRange}"
            );
        }

        public void MathStatisticsRangeStart(
            int Channel,
            int mnum,
            MathStatisticsRangeEnum mathStatisticsRange,
            double value
        )
        {
            ...
            string StatisticRange = Scpi.Format("{0}", mathStatisticsRange);
            ScpiCommand(
                $"CALCulate{Channel}:MEASure{mnum}:FUNCtion:DOMain:USER:STARt {StatisticRange}, {value}"
            );
        }

        public void MathStatisticsRangeStop(
            int Channel,
            int mnum,
            MathStatisticsRangeEnum mathStatisticsRange,
            double value
        )
        {
            ...
            string StatisticRange = Scpi.Format("{0}", mathStatisticsRange);
            ScpiCommand(
                $"CALCulate{Channel}:MEASure{mnum}:FUNCtion:DOMain:USER:STOP {StatisticRange}, {value}"
            );
        }
        #endregion
    }
