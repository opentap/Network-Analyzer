...
namespace OpenTap.Plugins.PNAX
{
    ...
    public partial class PNAX : ScpiInstrument
    {
        ...
        public void MODPowerSweepType(int Channel, MODSweepPowerTypeEnum powerType, int index = 1)
        {
            ...
            ScpiCommand(
                $"SENSe{Channel}:DISTortion:SWEep:POWer:CARRier:LEVel{index}:TYPE {MODPowerType}"
            );
        }
        ...
        {
            ScpiCommand(
                $"SENSe{Channel}:DISTortion:SWEep:POWer:CARRier:RAMP:LEVel{index}:STARt {value}"
            );
        }
        ...
        {
            ScpiCommand(
                $"SENSe{Channel}:DISTortion:SWEep:POWer:CARRier:RAMP:LEVel{index}:STOP {value}"
            );
        }
        ...
        public void MODSourceCorrection(
            int Channel,
            string port,
            MODSourceCorrectionEnum sourceCorrectionType
        )
        {
        ...
        public void MODCalEnable(int Channel, string port, bool state, MODCalTypeEnum MODCalType)
        {
            ...
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:ENABle {StateStr}, \"{port}\""
            );
        }

        public void MODCalPort(
            int Channel,
            string port,
            MODCalPortEnum MODCalPort,
            MODCalTypeEnum MODCalType
        )
        {
            string calType = Scpi.Format("{0}", MODCalType);
            string calPort = Scpi.Format("{0}", MODCalPort);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:RECeiver \"{calPort}\", \"{port}\""
            );
        }
        ...
        public void MODCalSpan(int Channel, string port, double CalSpan, MODCalTypeEnum MODCalType)
        {
            string calType = Scpi.Format("{0}", MODCalType);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:SPAN {CalSpan}, \"{port}\""
            );
        }

        public void MODCalGuardBand(
            int Channel,
            string port,
            double GuardBand,
            MODCalTypeEnum MODCalType
        )
        {
            string calType = Scpi.Format("{0}", MODCalType);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:GBANd {GuardBand}, \"{port}\""
            );
        }

        public void MODCalMaxIterations(
            int Channel,
            string port,
            int MaxIterations,
            MODCalTypeEnum MODCalType
        )
        {
            string calType = Scpi.Format("{0}", MODCalType);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:ITERations {MaxIterations}, \"{port}\""
            );
        }

        public void MODCalDesiredTolearance(
            int Channel,
            string port,
        ...
        )
        {
            string calType = Scpi.Format("{0}", MODCalType);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:{calType}:TOLerance {DesiredTolerance}, \"{port}\""
            );
        }

        public void MODCalibrationMeasure(
            int Channel,
            string port,
            MODCalSynchTypeEnum CalSynchType = MODCalSynchTypeEnum.SYNChronous
        )
        {
            string synchType = Scpi.Format("{0}", CalSynchType);
            ScpiCommand(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:ACQuire {synchType}, \"{port}\"",
                60000
            );
        }
        ...
        {
            return ScpiQuery(
                $"SOURce{Channel}:MODulation:CORRection:COLLection:ACQuire:STATus? \"{port}\""
            );
        }
        ...
        public void MODSetIBW(
            int Channel,
            double ibw,
            MODMeasConfigTypeEnum measConfType,
            int bnum = 1
        )
        {
        ...
        {
            return ScpiQuery<double>(
                $"SENSe{Channel}:DISTortion:TABLe:DATA:VALue? {bnum},\"{paramName}\""
            );
        }
        ...
        public void MODCorrelationApertureAuto(int Channel, bool state)
        {
            ...
            ScpiCommand(
                $"SENSe{Channel}:DISTortion:MEASure:CORRelation:APERture:AUTO:STATe {StateStr}"
            );
        }
