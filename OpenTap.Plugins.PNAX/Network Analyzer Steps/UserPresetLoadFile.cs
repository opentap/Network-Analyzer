...
{
    [Display(
        "User Preset Load File",
        Groups: new[] { "Network Analyzer" },
        Description: "Loads an existing instrument state file (.sta or .cst) to be used for User Preset. Subsequent execution of SYSTem:UPReset will cause the VNA to assume this instrument state."
    ...
    public class UserPresetLoadFile : TestStep
    {
        ...
        [Display(
            "User Preset State File",
            "Specfiy an existing instrument state file (.sta or .cst) to be loaded",
            "Load File",
            Order: 10
        )]
